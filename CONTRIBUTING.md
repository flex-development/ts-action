...
People interacting with the `ts-action` project are grouped into 4 categories:
...
```zsh
git clone https://github.com/flex-development/ts-action
cd grease
yarn bootstrap
```
...
```zsh
https://github.com/flex-development/ts-action/compare/next...<branch>
```
...
4. Open a new pull request from `release/*` into `next`
   ...
   - after all jobs are completed, the maintainer who approved the PR should
